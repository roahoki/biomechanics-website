...
export async function sendOrderStatusEmail(
  ...
  try {
    ...
    const statusBadgeColor = isConfirmed ? '#4CAF50' : '#f44336'
    const statusText = isConfirmed ? '✅ ENTREGADO' : '❌ ANULADO'
    const mainMessage = isConfirmed 
      ? 'Tu orden ha sido confirmada y entregada exitosamente.'
      : 'Tu orden ha sido anulada.'
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .header { border-bottom: 2px solid #7dff31; padding-bottom: 16px; margin-bottom: 24px; }
          h1 { margin: 0; color: #000; font-size: 24px; }
          .status { background: ${statusBadgeColor}; color: white; padding: 12px; border-radius: 4px; margin: 16px 0; text-align: center; font-weight: bold; }
          p { margin: 8px 0; color: #333; }
          table { width: 100%; margin: 16px 0; }
          .total { font-size: 18px; font-weight: bold; text-align: right; padding-top: 16px; border-top: 2px solid #7dff31; }
          .footer { margin-top: 24px; padding-top: 16px; border-top: 1px solid #eee; font-size: 12px; color: #999; }
    ...
    const result = await resendClient.emails.send({
      ...
      to: email,
      subject: isConfirmed 
        ? `Tu orden fue entregada - ${orderId.substring(0, 8)}`
        : `Tu orden fue anulada - ${orderId.substring(0, 8)}`,
      html
    })
